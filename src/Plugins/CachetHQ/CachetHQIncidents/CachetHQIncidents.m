...
#import "CachetHQIncidents.h"

#define kCachetHQBaseURL @"baseURL"

#import <asl.h>

@interface CachetHQIncidents () <NSMenuDelegate>
@property id<HITPluginsManagerProtocol> pluginsManager;
@property NSURL *incidentsAPI;
...
@implementation CachetHQIncidents
...
- (instancetype)initWithSettings:(NSDictionary*)settings
{
    ...
    if (self) {
        ...
        _incidentsAPI = [[NSURL URLWithString:[settings objectForKey:kCachetHQBaseURL]] URLByAppendingPathComponent:@"/api/v1/incidents"];
    }
    return self;
}
...
- (void)reloadStateAtPage:(NSInteger)requestedPage {
    NSURLComponents *urlComponents = [NSURLComponents componentsWithURL:self.incidentsAPI resolvingAgainstBaseURL:YES];
...
# warning Update to fit Incidents API
...
- (void)updateServiceGroup:(NSString*)serviceGroupKey withInfos:(NSDictionary*)infos {
    ...
    for (NSDictionary *serviceState in [infos objectForKey:@"enabled_incidents"]) {
